...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    for (const auto & armor : armors) {
      ...
      // 绘制世界坐标系数值
      tools::draw_text(
        img, fmt::format("armor_x: {:.2f}", armor.xyz_in_world[0]), {10, 600}, {0, 255, 0});
      tools::draw_text(
        img, fmt::format("armor_y: {:.2f}", armor.xyz_in_world[1]), {10, 630}, {0, 255, 0});
      tools::draw_text(
        img, fmt::format("armor_z: {:.2f}", armor.xyz_in_world[2]), {10, 660}, {0, 255, 0});
    }
    ...
    auto elapsed_time =
      std::chrono::duration_cast<std::chrono::milliseconds>(current_time - last_fps_time).count();
    ...
    cv::putText(
      img, fps_text, cv::Point(10, y_offset), font_face, font_scale, text_color, thickness);
    y_offset += line_height;

    // 运行模式
    std::string mode_text = "Mode: AutoAim";
    cv::putText(
      img, mode_text, cv::Point(10, y_offset), font_face, font_scale, text_color, thickness);
    y_offset += line_height;

    // 目标检测状态
    std::string detect_text = fmt::format("Detect: {}", has_target ? "YES" : "NO");
    cv::putText(
      img, detect_text, cv::Point(10, y_offset), font_face, font_scale,
    ...
    cv::putText(
      img, fire_text, cv::Point(10, y_offset), font_face, font_scale,
      plan.fire ? cv::Scalar(0, 0, 255) : text_color, thickness);
    y_offset += line_height;

    // 云台状态 - 接收到的数据
    std::string gimbal_status = fmt::format(
      "Gimbal Yaw: {:.2f}  Pitch: {:.2f}", -gs.yaw * 180.0 / M_PI, -gs.pitch * 180.0 / M_PI);
    cv::putText(
      img, gimbal_status, cv::Point(10, y_offset), font_face, font_scale, text_color, thickness);
    y_offset += line_height;

    std::string gimbal_vel = fmt::format(
      "Gimbal Vel Y: {:.2f}  P: {:.2f}", -gs.yaw_vel * 180.0 / M_PI, -gs.pitch_vel * 180.0 / M_PI);
    cv::putText(
      img, gimbal_vel, cv::Point(10, y_offset), font_face, font_scale, text_color, thickness);
    y_offset += line_height;

    // 规划状态 - 发送的数据
    std::string plan_status = fmt::format(
      "Plan Yaw: {:.2f}  Pitch: {:.2f}", -plan.yaw * 180.0 / M_PI, -plan.pitch * 180.0 / M_PI);
    cv::putText(
      img, plan_status, cv::Point(10, y_offset), font_face, font_scale, highlight_color, thickness);
    y_offset += line_height;

    std::string plan_vel = fmt::format(
      "Plan Vel Y: {:.2f}  P: {:.2f}", -plan.yaw_vel * 180.0 / M_PI,
      -plan.pitch_vel * 180.0 / M_PI);
    cv::putText(
      img, plan_vel, cv::Point(10, y_offset), font_face, font_scale, highlight_color, thickness);
    y_offset += line_height;

    std::string plan_acc =
      fmt::format("Plan Acc Y: {:.2f}  P: {:.2f}", plan.yaw_acc, plan.pitch_acc);
    cv::putText(
      img, plan_acc, cv::Point(10, y_offset), font_face, font_scale, highlight_color, thickness);
    y_offset += line_height;
    ...
    if (has_target) {
      ...
      cv::putText(
        img, target_info, cv::Point(10, y_offset), font_face, font_scale, cv::Scalar(255, 255, 0),
        thickness);
      y_offset += line_height;
      ...
      if (target.ekf_x().size() > 7) {
        ...
        cv::putText(
          img, target_w, cv::Point(10, y_offset), font_face, font_scale, cv::Scalar(255, 255, 0),
          thickness);
        y_offset += line_height;
      }
    }
    ...
    cv::line(
      img, cv::Point(img_width - 200, 20), cv::Point(img_width - 200, y_offset + 20),
    ...
    cv::putText(
      img, status_title, cv::Point(right_x, y_offset), font_face, font_scale + 0.1,
    ...
    cv::putText(
      img, time_text, cv::Point(right_x, y_offset), font_face, font_scale, text_color, thickness);
    y_offset += line_height;

    // 子弹速度
    std::string bullet_speed_text = fmt::format("Bullet Speed: {:.1f}", gs.bullet_speed);
    cv::putText(
      img, bullet_speed_text, cv::Point(right_x, y_offset), font_face, font_scale, text_color,
      thickness);
    y_offset += line_height;
    ...
    cv::putText(
      img, target_count_text, cv::Point(right_x, y_offset), font_face, font_scale,
    ...
    cv::putText(
      img, armor_count_text, cv::Point(right_x, y_offset), font_face, font_scale,
