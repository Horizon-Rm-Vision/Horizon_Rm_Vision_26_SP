...
// ============================================================================
// SVMClassifier implementation (ported from yolox_ov.cpp)
// ============================================================================

YOLOX_TRT::SVMClassifier::SVMClassifier(const std::string & model_path)
{
...
YOLOX_TRT::SVMClassifier::~SVMClassifier() { delete hog_; }

float YOLOX_TRT::SVMClassifier::getDistance(const cv::Point2f & p1, const cv::Point2f & p2)
{
  float x = (p1 - p2).x;
  float y = (p1 - p2).y;
...
bool YOLOX_TRT::SVMClassifier::affineNumber(
...
cv::Mat YOLOX_TRT::SVMClassifier::pixel(
...
cv::Mat YOLOX_TRT::SVMClassifier::autoGammaCorrect(const cv::Mat & image)
{
  cv::Mat img, dst;
  image.copyTo(dst);
  image.copyTo(img);
...
std::pair<int, double> YOLOX_TRT::SVMClassifier::predict(
  const cv::Mat & frame, const std::vector<cv::Point2f> & corners)
{
...
// ============================================================================
// LeNetClassifier implementation (ported from yolox_ov.cpp)
// ============================================================================
...
YOLOX_TRT::LeNetClassifier::LeNetClassifier(
...
bool YOLOX_TRT::LeNetClassifier::extractNumber(
  const cv::Mat & frame, const std::vector<cv::Point2f> & corners)
{
  static float classify_width_ratio = 0.2f;
  static float classify_height_ratio = 0.5f;
...
std::pair<ArmorName, double> YOLOX_TRT::LeNetClassifier::predict(
  const cv::Mat & frame, const std::vector<cv::Point2f> & corners)
{
...
// ============================================================================
// ResNetClassifier implementation (ported from yolox_ov.cpp)
// ============================================================================

YOLOX_TRT::ResNetClassifier::ResNetClassifier(const std::string & model_path)
{
  net_ = cv::dnn::readNetFromONNX(model_path);
  if (net_.empty()) {
...
bool YOLOX_TRT::ResNetClassifier::extractNumber(
  const cv::Mat & frame, const std::vector<cv::Point2f> & corners)
{
  static float classify_width_ratio = 0.2f;
  static float classify_height_ratio = 0.5f;
...
std::pair<ArmorName, double> YOLOX_TRT::ResNetClassifier::predict(
  const cv::Mat & frame, const std::vector<cv::Point2f> & corners)
{
  if (!extractNumber(frame, corners) || number_roi_.empty())
    return {ArmorName::not_armor, 0};
...
// ============================================================================
// YOLOX decode helpers (ported from yolox_ov.cpp / Horizon_Hero_Aim_26)
// ============================================================================
...
cv::Mat YOLOX_TRT::scaledResize(cv::Mat & img, Eigen::Matrix<float, 3, 3> & transform_matrix)
{
  float r =
...
void YOLOX_TRT::generate_grids_and_stride(
...
void YOLOX_TRT::generateYoloxProposals(
...
static inline float intersection_area(
  const YOLOX_TRT::ArmorObject & a, const YOLOX_TRT::ArmorObject & b)
{
...
static void qsort_descent_inplace(
  std::vector<YOLOX_TRT::ArmorObject> & objects, int left, int right)
{
  int i = left;
  int j = right;
...
static void qsort_descent_inplace(std::vector<YOLOX_TRT::ArmorObject> & objects)
{
...
static void nms_sorted_bboxes(
  std::vector<YOLOX_TRT::ArmorObject> & objects, std::vector<int> & picked,
  float nms_threshold, float merge_min_iou, float merge_conf_error)
{
  ...
  for (int i = 0; i < n; i++) {
    YOLOX_TRT::ArmorObject & a = objects[i];
    int keep = 1;
    for (int j = 0; j < static_cast<int>(picked.size()); j++) {
      YOLOX_TRT::ArmorObject & b = objects[picked[j]];
...
void YOLOX_TRT::sort_keypoints(std::vector<cv::Point2f> & keypoints)
{
...
// ============================================================================
// YOLOX_TRT main class
// ============================================================================

YOLOX_TRT::YOLOX_TRT(const std::string & config_path, bool debug)
: debug_(debug), detector_(config_path, false)
{
  auto yaml = YAML::LoadFile(config_path);

  model_path_ = yaml["yolox_trt_model_path"].as<std::string>();
  svm_model_path_ = yaml["yolox_svm_model_path"].as<std::string>();
  device_ = yaml["device"].as<std::string>();
  min_confidence_ = yaml["min_confidence"].as<double>();

<<<<<<< HEAD
  // Optional YOLOX-specific params (same structure as yolox_ov)
  if (yaml["yolox_trt"]) {
    auto yx = yaml["yolox_trt"];
=======
...
>>>>>>> origin/main
  ...
  if (label_correction_mode_ == "svm") {
    ...
    tools::logger()->info("YOLOX_TRT: using SVM label correction");
    ...
    tools::logger()->info("YOLOX_TRT: using LeNet label correction, threshold={}", lenet_threshold_);
  } else if (label_correction_mode_ == "resnet") {
    resnet_classifier_ = std::make_unique<ResNetClassifier>(resnet_model_path_);
    tools::logger()->info("YOLOX_TRT: using ResNet label correction");
  } else {
    tools::logger()->info("YOLOX_TRT: no label correction (using YOLOX built-in classes)");
  }
  ...
  try {
    trt_engine_->LoadEngine(model_path_);

    auto input_dim = trt_engine_->GetInputDim();
    auto output_dim = trt_engine_->GetOutputDim();

    // Input: NCHW format, dim[2]=H, dim[3]=W
    if (input_dim.size() >= 4) {
      input_h_ = static_cast<int>(input_dim[2]);
      input_w_ = static_cast<int>(input_dim[3]);
    }

    output_rows_ = static_cast<int>(output_dim[output_dim.size() - 2]);
    output_cols_ = static_cast<int>(output_dim[output_dim.size() - 1]);

    allocateGPUMemory();

    tools::logger()->info(
      "YOLOX_TRT: TensorRT engine loaded. Input: {}x{}, Output: {}x{}",
      input_h_, input_w_, output_rows_, output_cols_);
  } catch (const std::exception & e) {
    tools::logger()->error("YOLOX_TRT: Failed to load TensorRT engine: {}", e.what());
    throw;
  }
}
...
void YOLOX_TRT::allocateGPUMemory()
{
  size_t input_count = 3 * input_h_ * input_w_;
...
std::list<Armor> YOLOX_TRT::detect(const cv::Mat & raw_img, int frame_count)
{
  if (raw_img.empty()) {
    tools::logger()->warn("YOLOX_TRT: Empty image!");
  ...
  // CPU letterbox resize + BGR→RGB + float32 + NCHW
  cv::Mat pr_img = scaledResize(bgr_img, transform_matrix_);
  cv::Mat blob = cv::dnn::blobFromImage(pr_img, 1.0, cv::Size(input_w_, input_h_),
                                         cv::Scalar(0, 0, 0), false);

  // Upload to GPU and run inference
  size_t input_bytes = 3 * input_h_ * input_w_ * sizeof(float);
  CUDA_CHECK(cudaMemcpyAsync(d_input_tensor_, blob.ptr<float>(), input_bytes,
  ...
  // Decode YOLOX output
  std::vector<ArmorObject> objects;
  decodeOutputs(output_data.data(), objects, transform_matrix_);
  ...
  return parse(0 /* unused */, raw_img, frame_count, objects);
}

std::list<Armor> YOLOX_TRT::parse(
...
std::list<Armor> YOLOX_TRT::postprocess(
  double scale, cv::Mat & output, const cv::Mat & bgr_img, int frame_count)
{
  // For multi-threaded pipeline: decode raw output tensor directly
  std::vector<ArmorObject> objects;
...
bool YOLOX_TRT::check_name(const Armor & armor) const
{
  return armor.name != ArmorName::not_armor && armor.confidence > min_confidence_;
}

bool YOLOX_TRT::check_type(const Armor & armor) const
{
