...
- **功能更新:**删除trt模式的int8校准器；正式引入V7版通信模拟器;demo数据回归;补充部分相机内参;手眼标定程序C板通信替换为串口通信模式;迈德相机去除gamma调参;UI绘制数值单位由弧度修改为角度;增加曲线绘制功能(from infantryA);
- **质量更新:**修复通信和UI显示的数据问题,终端内容数据更新，迈德相机颜色修正
