...
#### Release Candidate 2 DEV2：

- **功能更新:**
  1.新增飞机前哨站优先机制,启用宏(PLANE_OUTPOST_PRIOR)并在yaml里启用后会将前哨站装甲板识别和追踪优先级设为最高,其他车亦可测试使用该功能
  2.新增飞机前哨站定时过滤机制,启用宏(PLANE_OUTPOST_TIMEFILTER)并在yaml里设定秒数后会在一定时间内在识别层过滤其他非前哨站装甲板,启用时UI增加计时器倒数剩余时间
  3.改进的autostart脚本,修复了自启并关闭脚本后无法再次启动脚本的问题
  4.为autostart脚本引入一键启动功能(autostart_v1/2.desktop),使用方法为修改desktop的Exec为正确的自启脚本位置,将desktop文件复制到桌面,右键选择允许执行即可拥有双击启动能力
- **质量更新:**
  1.recorder录制文件命名由时间改为序号,防止jetson应bios时间重置而记录混乱
- **待测试:**
  1.MPC模式的回放程序(auto_aim_test_mpc),可用性待测试
  2.由于mpc机制的特殊性,recorder原有录制内容无法满足新回放模式运行,现已修改recorder以适配该新模式,保留对aimer的旧回放程序的支持,可行性待测试
## 
