...
void ROS2::publish_status(uint8_t game_progress,uint16_t stage_remain_time,uint16_t current_hp,
  uint16_t ally_outpost_hp,uint8_t state,uint8_t energy_state,uint16_t bullets,uint8_t judge)
{
    publish2nav_->send_status(game_progress, stage_remain_time, current_hp,
       ally_outpost_hp, state, energy_state, bullets, judge);
}
...
std_msgs::msg::Int8 ROS2::subscribe_buff()
{
    return subscribe2nav_->subscribe_buff();
}
