...
Gimbal::Gimbal(const std::string & config_path)
{
  ...
  _mode_ = tools::read<int>(yaml, "_mode_");
...
bool Gimbal::open_serial()
{
  ...
  if (fd_ < 0) {
    tools::logger()->error(
      "[Gimbal] Failed to open serial port {}: {}", com_port_, strerror(errno));
    return false;
  }
...
void Gimbal::read_thread()
{
  ...
  while (!quit_) {
    if (error_count > 5000) {
      error_count = 0;
      tools::logger()->warn(
        "[Gimbal] Too many errors ({}), attempting to reconnect...", error_count);
      reconnect();
      continue;
    }
      ...
      if (buffer[i] == 0xCD) {
        ...
        if (i + packet_size <= data_index) {
          ...
          if (buffer[i + packet_size - 1] != 0xDC) {
            tools::logger()->warn(
              "[Gimbal] Packet tail mismatch, expected 0xDC, got 0x{:02x}",
              buffer[i + packet_size - 1]);
            continue;
          }

          auto t = std::chrono::steady_clock::now();
          static bool i_first=true; 
          // Copy valid packet
          ...
          if (mode <= 3) {
            ...
            // 使用局部变量记录解析后的数据内容
            tools::logger()->info(
              "[Gimbal] Parsed read data - Mode: {}->{}, Pitch: {:.3f}, Yaw: {:.3f}, "
              "BulletSpeed: {}, Quaternion: [{:.3f}, {:.3f}, {:.3f}, {:.3f}]",
              str(old_mode), str(mode_), -pitch * (180.0 / M_PI), -yaw * (180.0 / M_PI),
              bullet_speed, q.w(), q.x(), q.y(), q.z());
            ...
            tools::logger()->warn(
              "[Gimbal] Skipping packet with invalid mode: {}, raw data: {}", mode,
              packet_to_hex(buffer + i, packet_size));
          ...
          tools::logger()->trace(
            "[Gimbal] Incomplete packet, have {} bytes, need {} bytes", data_index - i,
            packet_size);
          break;
        }
      }
    ...
    if (!packet_found && data_index >= sizeof(buffer) - 10) {
      tools::logger()->warn(
        "[Gimbal] Buffer overflow protection, clearing buffer. data_index: {}", data_index);
